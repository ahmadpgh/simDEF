# This code extracts parents of GO terms included in CC ontology (Please read the read-me file before use!)
use IO::File;
...
open(my $w, '>', "../../Extra/CC/CC_GO_parents.dat") or die "Could not open file to write in\n";
...
foreach(<$r>){
	...
	if (!($_=~/^\[Term\]/) and ($flag == 1)){
		...
		if(($_=~/^is_a: GO:/) and ($def_flag == 1) and ($GO_hir eq "CC")){
			my @b = split /^is_a: GO:/;
			my @c = split " ", $b[1];
