# This code extracts definitions of GO terms included in MF ontology (Please read the read-me file before use!)
use IO::File;
...
open(my $w, '>', "../../Extra/MF/MF_GO_definition.dat") or die "Could not open file to write in\n";
...
foreach(<$r>){
	...
	if (!($_=~/^\[Term\]/) and ($flag == 1)){
		...
		if(($_=~/^def: "/) and ($def_flag == 1) and ($GO_hir eq "MF")){
			my @b = split /^def: "/;
			my @c = split /\" \[/, $b[1];
			$hash{$GO_id} = $c[0];
		}
